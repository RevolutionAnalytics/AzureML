...
api <- updateWebService(
  ...
  serviceId = api$WorkspaceId   # <<-- Note you must add wsid to update!
)
```
...
The discovery functionality in this package allows users to explore and obtain
the web services available to their workspace. On the highest level, the user
can use their workspace ID and authorization token, both of which were
described in the preceding sections, to obtain a list of web services available
to that workspace:
