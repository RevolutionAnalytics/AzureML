...
Functions can depend on arbitrary package or object. However, currently functions are constrained to take in primitive data types, e.g. ints, strings, etc., meaning the function to be published cannot take in dataframes, lists, etc.
...
- Third and fourth, we pass the input and output schema. These are named lists of the format: `<parameter name> = <type>`.
  - For the *input schema*, this should match the signature of the function to be published, so the list length should equal the number of parameter the function takes in.
  - For the *output schema*, the names of the parameters can vary, but should also match the number of expected outputs. The output schema names will be the names of the columns returned when the web service is consumed. For both schemas, the following types are supported:
